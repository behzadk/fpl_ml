...
def _merge_additional_raw_data(gw_df: pd.DataFrame, player_raw_df: pd.DataFrame, player_id_df, master_team_df: pd.DataFrame, season: str):
...
def _generate_player_summary_statistics(master_df: pd.DataFrame):
    ...
    for player_name in master_df.player_name.unique():
        ...
        player_df = generate_moving_average(player_df, column='minutes', window_size=3, fill_value=-1)
        player_df = generate_moving_average(player_df, column='assists', window_size=3, fill_value=-1)
        player_df = generate_moving_average(player_df, column='goals_conceded', window_size=3, fill_value=-1)
        player_df = generate_moving_average(player_df, column='goals_scored', window_size=3, fill_value=-1)
        player_df = generate_moving_average(player_df, column='bonus', window_size=3, fill_value=-1)
...
def _clean_player_names(df):
    ...
    df['player_name'] = df.apply(lambda row: row['player_name'].replace(f'_{str(row.player_id)}', ''), axis=1)

    # Replace white space with undescores
    df['player_name'] = df.apply(lambda row: row['player_name'].replace(' ', '_'), axis=1)
...
def _clean_game_weeks(df, game_week_column, season_column):
    ...
    for season in df[season_column].unique():
        ...
        season_df[game_week_column] = season_df[game_week_column].replace(remapped_game_weeks)
...
def generate_moving_average(player_season_df, column, window_size, fill_value=None):
    player_season_df[f'rolling_mean_{window_size}_{column}'] = player_season_df[column].rolling(window=window_size).mean().shift(fill_value=fill_value)
    player_season_df[f'rolling_mean_{window_size}_{column}'] = player_season_df[f'rolling_mean_{window_size}_{column}'].fillna(fill_value)
...
def main():
    ...
    game_week_paths_df = game_week_paths_df.loc[game_week_paths_df['season'] == '2019-20']
    ...
    for idx, row in game_week_paths_df.iterrows():
        ...
        gw_df = _load_gw_data(game_week_path=row["game_week_path"], game_week=row['game_week'])
    ...
    master_df = _clean_game_weeks(master_df, game_week_column='game_week', season_column='season')

    master_df['starting_year'] = master_df.apply(lambda row: get_season_start_year(row['season']), axis=1)
