...
class TestFPLEnv(unittest.TestCase):
    def setUp(self):
        ...
        for i in range(2):
            ...
            base_agent.init_team(self.season_df[self.season_df[NumericalFeatures.GAME_WEEK] == 1])
            self.agent_n.append(base_agent)
        
        self.env = FPLEnv(self.agent_n, self.season_df)

    def test_step(self):
        obs, reward_n, done = self.env.step(teams=[agent.team for agent in self.agent_n])
        print(reward_n, done)
